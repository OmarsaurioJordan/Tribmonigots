...
repeat total {
    ...
    zz = s_mizona(xx, yy);
    if random(1) < s_densidad_minerales(zz) {
        if random(1) < cluster and instance_exists(o_mineral) {
            repeat 4 {
                cerca1 = instance_nearest(xx, yy, o_mineral);
                cerca1 = point_distance(xx, yy, cerca1.x, cerca1.y);
                xxx = xx + random_range(-200, 200);
                yyy = yy + random_range(-200, 200);
                cerca2 = instance_nearest(xxx, yyy, o_mineral);
